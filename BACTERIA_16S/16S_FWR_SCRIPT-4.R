...
bacterial.model <- ggplot(data=obs.df) +
...
# Increase the plot margins
bacterial.model  <- bacterial.model  +
  theme(plot.margin = margin(30, 10, 10, 10, "pt"))  # Adjust the margins as needed

# Create a ggdraw object
plot_bacterial.model  <- ggdraw() +
  draw_plot(bacterial.model) +  # Add the plot
  draw_label("16S Neutral Model", fontface = "bold", size = 18, x = 0.5, y = 0.98) +  # Add the title
...
above16S.model <- ggplot(data=obs.df) +
  geom_point(data=obs.df, aes(x=log10(metacomm_RA), y=frequency), 
             alpha=.4, size=3, color="#37AEC3") +
  geom_line(data=pred.df, aes(x=log10(metacomm_RA), y=frequency), color="red") + 
  geom_line(data=pred.df, aes(x=log10(metacomm_RA), y=frequency_lowerCI), linetype=2, color="red") + 
...
above16S.model  <- above16S.model +
  theme(plot.margin = margin(30, 10, 10, 10, "pt"))  # Adjust the margins as needed

# Create a ggdraw object
plot_above16S.model  <- ggdraw() +
  draw_plot(above16S.model) +  # Add the plot
  draw_label("16S Above ground", fontface = "bold", size = 18, x = 0.5, y = 0.98) +  # Add the title
  draw_label("C", size = 18, x = 0.025, y = 0.98, fontface = "bold")  # Add the tag "A"
...
below16S.model <- ggplot(data=obs.df) +
  geom_point(data=obs.df, aes(x=log10(metacomm_RA), y=frequency), 
             alpha=.4, size=3, color="#37AEC3") +
  geom_line(data=pred.df, aes(x=log10(metacomm_RA), y=frequency), color="red") + 
  geom_line(data=pred.df, aes(x=log10(metacomm_RA), y=frequency_lowerCI), linetype=2, color="red") + 
  ...
  geom_text(data=fitstats.b, aes(label = paste("R^2 == ", round(Rsqr, 3))), 
            x=-5.15, y=0.96, size=5, parse=TRUE) +
  geom_text(data=fitstats.b, aes(label = paste("italic(m) ==", round(m, 3))), 
            x=-5.15, y=0.88, size=5, parse=TRUE) + 
  labs(x = expression(Log[10](Mean~relative~abundance)), y="Ocurrence frequency") +
  theme_bw(base_size = 32, base_family = "Times") +
  theme(legend.position = "none",axis.title = element_text(size=20),axis.text = element_text(size=20))+
  ylim(0,1)  # Set the x-axis limits

below16S.model  <- below16S.model +
  theme(plot.margin = margin(30, 10, 10, 10, "pt"))  # Adjust the margins as needed

# Create a ggdraw object
plot_below16S.model  <- ggdraw() +
  draw_plot(below16S.model) +  # Add the plot
  draw_label("16S Below ground", fontface = "bold", size = 18, x = 0.5, y = 0.98) +  # Add the title
  draw_label("D", size = 18, x = 0.025, y = 0.98, fontface = "bold")  # Add the tag "A"
